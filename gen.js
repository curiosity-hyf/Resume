var resumeMarkdownParser = function (id, rawInput) {
    ...
    for (var i = 0; i < _ast.length; i++) {
        ...
        if (_ast[i].beginWith('@ ')) {
			...
			_htmlText = _ast[i].slice(3).replace(/(.*)?(:|：) /i, '<em>$1</em>:').replace(/[:|：](.*)?/i, '<strong>$1</strong>')._parseMarkdownAnchor();
		} else if (_ast[i].beginWith('# ')) {
			_htmlTag = 'h2';
		...
		document.getElementById(id).appendChild(_htmlElementGenerated);
